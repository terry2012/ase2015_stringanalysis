...
import edu.buffalo.cse.blueseal.utils.Debug;

public class BSInterproceduralAnalysis extends AbstractInterproceduralAnalysis {

	static Logger logger = Logger.getLogger(BSInterproceduralAnalysis.class);
	...
	protected void analyseMethod(SootMethod method, Object dst) {
		logger.debug("Meth name is: " + method.getName());
		if(method.getName().contains("urlStringAsStringThreeArgumentConstructor")){
			XStream xstream = new XStream();
			String xml = xstream.toXML(method);
			File file = new File("resources" + File.separator + "case4" + File.separator + method.getName() +".xml");
			try{
