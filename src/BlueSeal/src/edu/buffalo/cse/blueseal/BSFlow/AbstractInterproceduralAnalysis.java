...
import edu.buffalo.cse.blueseal.blueseal.AnotherShimpleTransformer;
import soot.*;
...
public abstract class AbstractInterproceduralAnalysis {
	...
	static Logger logger = Logger.getLogger(AbstractInterproceduralAnalysis.class);
     ...
     * 
     * applySummary may affect intraprocedural analysis,too
     *
     ...
     * here
     * @param dst where to put the result for intraprocedural analysis
     * @param summary2 where to put the result for interprocedural analysis
     * @see analyseCall
     */
    protected abstract void applySummary(Object src,
                     Stmt   callStmt,
                     Object summary,
                     Object dst, Object summary2,
                     Object callMethod, Map map);
    
    /*
     * 05/10/2013, Feng
     * new applysummary, interprocedural analysis will use this method to 
     * update callee method's summary
     * 
     * applySummary may affect intraprocedural analysis, too
     * 
     * @param src //src summary valid before call statement, this is from intraprocedural analysis
     * @param callStmt current analyzed statement containing a InvokeStmt or InvokeExpr
     * @param summary inter procedural method summary for method of InvokeStmt
     * @param dst output summary for intra procedural analysis(here is set of values that need to
     * 			be added to intra flowSet
     * @param intra procedural analysis instance used to analyze current method
     */
    protected abstract void applySummary(Object src, 
    		Stmt callStmt,
    		Object summary,
    		Object dst, 
    		BSForwardFlowAnalysis intra);
    ...
    /*
     * 05/10/2013 Feng
     * Analyze the callStmt, this will be call by intra procedural analysis
     * this will call applySummary
     * there will be two things may be updated
     * 1. for intra procedural analysis, this may update the out flowSet,
     * 		which will be put in dst
     * 2. for inter procedural analysis, this will update the current method's 
     * 		summary(BSG of current method)
     * @param src the in set from intra procedural anlysis, actually we are not using it, just for completeness
     * @param callStmt statement we need to analyze
     * @param dst set of values that needs to add into intra procedural analysis flowSet
     * @param intra, intra procedural analysis instance used to analyze current method
     */
    protected void analyseCall(Object src,
            Stmt   callStmt,
            Object dst, 
    		BSForwardFlowAnalysis intra){
    	logger.debug("In analyseCall in AbstractInterproceduralAnalysis");
	    ...
	    while (it.hasNext()) {
	        ...
	        applySummary(src, callStmt, elem, dst, intra);
	    }
    }
 
   /**
     * Analyze the call callStmt in the context src, and put the result into 
     * dst.
     * This will repeatedly calling summaryOfUnanalysedMethod and applySummary,
     * and then merging the results using merge.
     * @param this summary is for interprocedural analysis
     * @see summaryOfUnanalysedMethod
     * @see applySummary
     */
    protected void analyseCall(Object src,
                   Stmt   callStmt,
                   Object dst, Object summary, Object callMethod,
                   Map unitToNode)
    {
	    Object accum = new HashSet();
	    Iterator it = cg.edgesOutOf(callStmt);
	    //copy(accum, dst);
	    while (it.hasNext()) {
	        Edge edge = (Edge)it.next();
	        ...
	        applySummary(src, callStmt, elem, accum, summary, callMethod, unitToNode);
	        ((Set)dst).addAll((Set)accum);
	        //merge(dst, accum, dst);
	    }
    }
    ...
    protected void doAnalysis(boolean verbose)
    {
	    ...
	    while (!queue.isEmpty()) {
	        ...
	        if (((Integer)nb.get(m)).intValue() < 4) {  
	...
	public void analyseCall(Object in, Stmt stmt, Object values,
			BSBackwardFlowAnalysis bsBackwardFlowAnalysis) {
	    ...
	    while (it.hasNext()) {
	        ...
	        applySummary(in, stmt, elem, values, bsBackwardFlowAnalysis);
	    }
	}

	public abstract void applySummary(Object in, Stmt stmt, Object elem,
			Object values, BSBackwardFlowAnalysis bsBackwardFlowAnalysis);
}
