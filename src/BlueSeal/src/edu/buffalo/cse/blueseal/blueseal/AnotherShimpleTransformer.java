...
public class AnotherShimpleTransformer extends SceneTransformer {
	...
	static Logger logger = Logger.getLogger(AnotherShimpleTransformer.class);
				...
				if (stmt.containsInvokeExpr()) {
					...
					if (methodRef.declaringClass().getName().contains("ContentResolver")
						...
						if (methodRef.name().contains("insert")
							...
							logger.debug(stmt.getUseBoxes().get(0).getValue());
							logger.debug(stmt.getUseBoxes().get(1).getValue());
